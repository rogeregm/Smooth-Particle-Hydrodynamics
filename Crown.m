function [particles, rho_0,gamma,c_0,p_0,Xi,my,alpha, a_wall, int_fluid, int_boundary] = Crown(kernel, dx, d, v_max, alpha)
    %% first fluid phase
    ...
    spc = (0.1/(dx));
    for nn=1:spc % point
        pp = 6*nn;%points per circle
        for kk = 1:pp %angle
            angl = (kk-1)*(360/pp);
            %x-coord
            fluid((nn-1)*pp+kk,1) = (nn)*(dx)*cosd(angl);
            %y-coord
            fluid((nn-1)*pp+kk,2) = 0.6+(nn)*(dx)*sind(angl);   
        end
    end
%     fluid(1,1) = dx;
%     fluid(1,2) =0.6;
    newfluid(:,:) = fluid(1:end,:);
%   fluid = create_fluid(dx, f_lowleft, f_lowright, f_upleft, f_upright);
    [particles, int_f2] = initialisation(particles, newfluid, flag, rho_0(flag), dx, d, vel);
